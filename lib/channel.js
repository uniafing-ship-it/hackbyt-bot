...
function normalize(text) {
  return text.toLowerCase().replace(/https?:\/\/\S+/g, ' ').replace(/[^a-zа-яё0-9\s]/gi, ' ').replace(/\s+/g, ' ').trim();
}
...
export function findSimilarPosts(topic, posts, limit = 8) {
  return posts.map((post) => ({ ...post, score: similarity(topic, post.text) })).filter((post) => post.score > 0).sort((a, b) => b.score - a.score).slice(0, limit);
}
